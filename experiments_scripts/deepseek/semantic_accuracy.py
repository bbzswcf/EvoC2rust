...
from code_optim.code_gen import  code_generation, code_verification, blankfill_test
...
if __name__ == "__main__":
    ...
    all_proj_total_accept_lines_cnt = 0
    all_proj_total_gold_lines_cnt = 0
    all_proj_total_generated_lines = 0
    ...
    for project_name in projects:          
        ...
        cache_0 = ProjectCache(config, cache_dir="./data/fill/cache/evo-c2rust-v2-ds-gen-only")
        cache_1 = ProjectCache(config, cache_dir="./data/fill/cache/evo-c2rust-v2-ds-delim-fix")
        cache_2 = ProjectCache(config, cache_dir="./data/fill/cache/evo-c2rust-v2-ds-rule-fix")
        cache_3 = ProjectCache(config, cache_dir="./data/fill/cache/evo-c2rust-v2-ds-llm-repair")
        gold_cache = ProjectCache(config, cache_dir="./data/fill/gold_cache")

        code_generation(config, rust_metadata, cache_0, client, multi_process=True, threads_num=5)
        rust_metadata = c_metadata_to_rust_metadata(config)
        ...
        report_3 = code_optimization(config, rust_metadata, cache_2, cache_3, client,
            optimizations={
        ...
        rust_metadata = c_metadata_to_rust_metadata(config)
        with open(os.path.join("./data/fill/testcases", project_name, "mod.rs"), "r") as f:
            mod_rs_content = f.read() 
        rust_metadata.set_mod_rs(mod_rs_content)
        report_4 = blankfill_test(config, rust_metadata, gold_cache, cache_3)

        items = ["macro", "macro_function", "definition", "function"]
        calcs = ["all_cnt", "passed_cnt", "pass_rate"]
        proj_report = {item: {calc: report_4[item][calc] for calc in calcs} for item in items}
